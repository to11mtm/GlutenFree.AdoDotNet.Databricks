...
- **Complex types**: verified against a live warehouse — with ARROW_STREAM the API delivers `ARRAY`/`MAP`/`STRUCT` as **genuine Arrow nested arrays** (List/Map/Struct), not JSON strings. The provider serializes them to JSON strings per the v1 type mapping (arrow-dotnet's nested-type support *is* load-bearing and works).
